  ...
  return <AppBackground style={s.screen}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void fetchHealth()} tintColor={colors.primary} />}>
      ...
      {!data ? <View style={s.empty}><Activity size={32} color={colors.mutedForeground} /><Text style={[s.emptyText, { color: colors.foreground }]}>No server data</Text><TouchableOpacity onPress={() => void fetchHealth(true)} style={s.retry}><Text style={s.retryText}>Try Again</Text></TouchableOpacity></View> : <>
        ...
        <FrostedView intensity={52} tint="dark" style={s.card}><Info label="Artwork files" value={String(application.artworkTotal || 0)} icon={<Archive size={16} color={colors.primary} />} /><Info label="Tracked file size" value={formatBytes(application.storageUsed)} icon={<HardDrive size={16} color={colors.info} />} /><Info label="System uptime" value={`${Math.floor((server.uptime || 0) / 86400)}d ${Math.floor(((server.uptime || 0) % 86400) / 3600)}h`} icon={<Clock size={16} color="#a78bfa" />} /></FrostedView>
      </>}
    </ScrollView>
  </AppBackground>;
}

function Metric({ title, value, detail, icon }: { title: string; value: string; detail: string; icon: React.ReactNode }) { return <FrostedView intensity={52} tint="dark" style={s.metric}><View style={s.cardTop}><Text style={s.cardTitle}>{title}</Text>{icon}</View><Text style={s.metricValue}>{value}</Text><Text style={s.cardSub}>{detail}</Text></FrostedView>; }
function Service({ title, detail, value, icon }: { title: string; detail: string; value: string; icon: React.ReactNode }) { const { colors } = useTheme(); const online = ['READY', 'ACTIVE', 'ONLINE', 'Connected'].includes(value); return <FrostedView intensity={52} tint="dark" style={s.metric}><View style={s.cardTop}>{icon}<Text style={s.cardTitle}>{title}</Text></View><Text style={s.cardSub} numberOfLines={1}>{detail}</Text><Text style={[s.status, { color: online ? colors.success : colors.warning }]}>{value}</Text></FrostedView>; }
