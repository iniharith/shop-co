      ...
      <div className="max-w-3xl mx-auto">
        ...
        {uploadSuccess ? (
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-10 text-center">
            ...
            <h2 className="text-2xl font-bold text-emerald-400 mb-2">Fail Berjaya Dihantar!</h2>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all"
              />
            </div>
            ...
            >
              ...
              <p className="text-base font-semibold text-white mb-1">Seret fail ke sini atau klik untuk pilih</p>
            ...
            {queue.length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                ...
                <div className="space-y-3">
                  {queue.map(q => (
                    <div key={q.id} className="flex items-center gap-3 bg-white/5 rounded-xl p-3">
                      {q.preview ? (
                        <img src={q.preview} alt={q.file.name} className="w-12 h-12 object-cover rounded-lg flex-shrink-0" />
                      ) : (
                        <div className="w-12 h-12 bg-white/10 rounded-lg flex items-center justify-center text-2xl flex-shrink-0">📄</div>
                      )}
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-white truncate">{q.file.name}</p>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all resize-none"
              />
            </div>
            ...
            <button
              ...
              className={`
                ...
                ${uploading || !queue.length
                  ? 'bg-purple-900/50 text-purple-400/50 cursor-not-allowed'
                  : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-lg shadow-purple-500/25 hover:-translate-y-0.5'
                }
              `}
            >
              {uploading ? (
        ...
        <div className="mt-12">
          ...
          {loadingFiles ? (
            <div className="space-y-3">
              {[1,2,3].map(i => (
                <div key={i} className="bg-white/5 animate-pulse h-16 rounded-xl" />
              ))}
            </div>
          ) : myFiles.length === 0 ? (
            ...
            <div className="space-y-3">
              {myFiles.map(f => (
                <div key={f._id} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
