...
export function loadTemplateArtwork(template: PhotoCanvasTemplate) {
  if (template.artboards?.length) {
    ...
    if (!artwork.has(key)) artwork.set(key, Promise.all(template.artboards.map(page => fetch(page.svg).then(r => r.text()))).then((pages) => {
      ...
      const body = pages.map((source, index) => { const doc = new DOMParser().parseFromString(source, 'image/svg+xml'); const root = doc.documentElement; const offset = template.artboards!.slice(0, index).reduce((sum, page) => sum + page.height, 0); const ids = template.artboards![index].slots.map(slot => slot.id); let inner = root.innerHTML; ids.forEach(id => { inner = inner.replaceAll(`kc-${id}`, `kc-artboard-${index + 1}-${id}`); }); return `<g transform="translate(0 ${offset})">${inner}</g>`; }).join('');
