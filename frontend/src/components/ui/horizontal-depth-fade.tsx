 ...
 * Source: https://21st.dev/@unlumen/components/horizontal-depth-fade (MIT)
 * Built by leo — https://x.com/leouiux (Unlumen UI)
 * Ported with an added `renderItem` extension so the strip can render
 * arbitrary cards (e.g. ProductCard) instead of a plain background image.
 */
"use client";
...
export interface HorizontalDepthFadeProps {
  ...
  /** Optional custom renderer for each strip item (overrides the background-image figure). */
  renderItem?: (image: HorizontalDepthFadeImage, index: number) => ReactNode;
          ...
          >
            {images.map((img, i) => (
              <figure
                key={i}
                className={cn(
                  "hbs-item relative z-10 m-0 shrink-0",
                  renderItem ? "" : "overflow-hidden rounded-xl",
                )}
