...
const ChatBox = ({ userId }: { userId: string }) => {
  ...
  let conversation = conversations[0]; // Assuming customer has 1 main support conversation
  ...
  }, [conversations, convLoading, createConv, userId, isCreating]);

  if (convLoading || isCreating || !conversation) {
    return <div className="flex-1 flex items-center justify-center text-sm text-gray-500">Connecting...</div>;
  }

  return <ChatMessages conversationId={conversation._id} currentUserId={userId} />;
};
