  ...
  const handleDownloadAll = async (group: any, e: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/files/download-batch`, {
        ...
        body: JSON.stringify({ fileIds, zipName: group.folderName || "production", downloadId }),
      });
  ...
  if (!groupedFromServer.length && folderGroupError) {
    return (
      <div className="p-8 text-center border border-dashed rounded-xl bg-card space-y-3">
        <p className="text-sm text-muted-foreground">Production files could not be loaded.</p>
    ...
    <div className="space-y-6 bg-background/40 backdrop-blur-md rounded-2xl border border-white/10 shadow-xl p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-2 w-full max-w-md">
          ...
          <Button variant="outline" size="icon" onClick={() => refetch()} disabled={isFetching} className="shrink-0" title="Refresh Production Files">
      ...
      {groupedFiles.length === 0 ? (
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:h-[calc(100vh-220px)] lg:min-h-[600px]">
          ...
          {/* RIGHT PANEL (DETAIL) */}
          <div className="w-full lg:w-2/3 xl:w-3/4 min-w-0 border rounded-xl bg-card shadow-sm flex flex-col overflow-hidden lg:h-full">
            {selectedFolder ? (() => {
              ...
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8 text-center space-y-5 bg-muted/10">
                ...
                <div>
                  ...
                  <p className="text-sm max-w-sm mx-auto text-muted-foreground/80 leading-relaxed">
                    Click on a folder from the list on the left to view its task details, update its status, and download files for production.
                  </p>
                </div>
              </div>
            )}
