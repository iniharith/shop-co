...
const t = {
  en: {
    ...
    addressPlaceholder: "Delivery / collection address (optional)",
    itemLabel: "Item",
  ...
  ms: {
    ...
    addressLabel: "Alamat",
    addressPlaceholder: "Alamat penghantaran / ambilan (pilihan)",
    itemLabel: "Item",
...
export default function ManualUploadPortal() {
  ...
  const uploadOneFile = async (itemId: string, index: number): Promise<FileState["uploaded"] | null> => {
    ...
    try {
      ...
      const urlRes = await fetchWithTimeout(`${BACKEND}/api/files/manual/upload-url`, {
        method: "POST",
  ...
  const handleUpload = async () => {
    ...
    try {
      const metaRes = await fetchWithTimeout(`${BACKEND}/api/files/manual/save-metadata`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="min-h-[100dvh] bg-black text-white flex flex-col items-center justify-center p-4 pb-24">
      <div className="mb-8 flex flex-col items-center justify-center gap-4">
        <div className="flex items-center justify-center gap-2">
          <img src="/logo.png" alt="Logo" className="w-8 h-8 rounded" onError={(e) => e.currentTarget.style.display = 'none'} />
          <h1 className="text-xl font-bold tracking-tight">Kampung Cetak <span className="text-white/40 font-normal">· {langDict.pageTitle}</span></h1>
        </div>
