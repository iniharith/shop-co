...
import { AlertTriangle, Archive, BarChart3, Check, Eye, History, PackagePlus, Pencil, Plus, RefreshCw, Search, Trash2, TrendingUp, TriangleAlert, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { adjustCatalogStock, archiveCatalogProduct, bulkArchiveCatalog, bulkDeleteCatalog, getCatalog, getCatalogAnalytics, getCatalogStockAdjustments, syncPublishedCatalogPricing } from '@/api/catalog';
import { resolveImages } from '@/utils/productImage';

type SizeStock = { size: string; stock: number; lowStockThreshold?: number };
type Product = { _id: string; name: string; description: string; category: string; price: number; images?: string[]; isDelete?: boolean; status?: 'draft' | 'published' | string; slug?: string; sizes?: SizeStock[]; viewCount?: number; unitsSold?: number; updatedAt?: string; updatedBy?: string; size?: string; stock?: number };
...
export default function CatalogPage() {
  ...
  const performanceCards = [
    { title: 'Most viewed', icon: Eye, products: analytics.mostViewed, metric: (product: Product) => `${product.viewCount || 0} views` },
    { title: 'Best selling', icon: TrendingUp, products: analytics.bestSelling, metric: (product: Product) => `${product.unitsSold || 0} sold` },
    { title: 'Recently updated', icon: RefreshCw, products: analytics.recentlyUpdated, metric: (product: Product) => product.updatedAt ? new Date(product.updatedAt).toLocaleString() : 'No date' },
    { title: 'Low stock', icon: TriangleAlert, products: analytics.lowStock, metric: (product: Product) => `${product.stock || 0} left` },
  ];
  ...
  const syncPublishedPricing = async () => {
    setSyncingCatalog(true);
    try {
      const result = await syncPublishedCatalogPricing(token);
      toast.success(`Catalog synced: ${result.updated || 0} updated, ${result.added || 0} added.`);
      await load();
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Could not synchronize the published catalog');
    } finally {
  ...
  return <main className="space-y-6 p-4 md:p-8">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Storefront / Catalog</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Product catalog</h1><p className="mt-2 text-sm text-muted-foreground">Manage the storefront, publishing workflow, and inventory from one catalog.</p></div><div className="flex gap-2"><Button variant="outline" disabled={syncingCatalog || !token} onClick={() => void syncPublishedPricing()}><RefreshCw className={`mr-2 ${syncingCatalog ? 'animate-spin' : ''}`} />{syncingCatalog ? 'Syncing pricing...' : 'Sync published pricing'}</Button><Button asChild><Link href="/admin/catalog/new"><Plus className="mr-2" /> New product</Link></Button></div></div>
    ...
    <section className="space-y-3"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" /><div><h2 className="text-xl font-semibold">Catalog performance</h2><p className="text-sm text-muted-foreground">Demand, stock, and recent catalog activity at a glance.</p></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{performanceCards.map(({ title, icon: Icon, products: items, metric }) => <Card key={title}><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-base">{title}</CardTitle><Icon className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent className="space-y-2">{items.slice(0, 4).map(product => <Link key={`${title}-${product._id}-${product.size || ''}`} href={`/admin/catalog/${product._id}`} className="flex items-center gap-2 rounded-md p-1 hover:bg-muted"><div className="h-8 w-8 shrink-0 overflow-hidden rounded bg-muted">{product.images?.[0] && <img src={product.images[0]} alt={product.name} className="h-full w-full object-cover" />}</div><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{product.name}</span><span className="block truncate text-[11px] text-muted-foreground">{title === 'Recently updated' ? `by ${product.updatedBy || 'System'} · ${metric(product)}` : metric(product)}</span></span></Link>)}{items.length === 0 && <p className="text-sm text-muted-foreground">No data yet.</p>}</CardContent></Card>)}</div></section>
