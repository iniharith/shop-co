...
const normalizeProduct = (body) => ({
    ...
    sizes: Array.isArray(body.sizes)
        ? body.sizes.map((item) => ({
            ...
            lowStockThreshold: (item === null || item === void 0 ? void 0 : item.lowStockThreshold) === '' || (item === null || item === void 0 ? void 0 : item.lowStockThreshold) == null ? 10 : Number(item.lowStockThreshold),
            images: Array.isArray(item === null || item === void 0 ? void 0 : item.images) ? item.images.map(String).filter(Boolean) : [],
    ...
    packageContents: Array.isArray(body.packageContents) ? body.packageContents.map((item) => String(item)).filter(Boolean) : [],
...
router.get('/analytics', (_req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [mostViewed, sales, lowStock, recentlyUpdated] = yield Promise.all([
            product_model_1.default.find(activeProducts).sort({ viewCount: -1, updatedAt: -1 }).limit(10).select('name category slug images viewCount updatedAt sizes').lean(),
            order_model_1.default.aggregate([
                { $match: { isDeleted: { $ne: true }, orderStatus: { $nin: ['CANCELLED', 'FAILED', 'RETURNED'] } } },
                { $unwind: '$products' },
                { $group: { _id: '$products.product', unitsSold: { $sum: '$products.quantity' }, revenue: { $sum: { $ifNull: ['$products.lineTotal', '$products.price'] } } } },
            ...
            product_model_1.default.aggregate([
                ...
                { $unwind: '$sizes' },
                { $match: { $expr: { $lte: ['$sizes.stock', { $ifNull: ['$sizes.lowStockThreshold', 10] }] } } },
                { $project: { name: 1, category: 1, slug: 1, images: 1, size: '$sizes.size', stock: '$sizes.stock', lowStockThreshold: { $ifNull: ['$sizes.lowStockThreshold', 10] }, updatedAt: 1 } },
                { $sort: { stock: 1, updatedAt: -1 } },
                { $limit: 25 },
            ]),
            product_model_1.default.find(activeProducts).sort({ updatedAt: -1 }).limit(10).select('name category slug images updatedAt viewCount').lean(),
        ]);
        const productById = new Map((yield product_model_1.default.find({ _id: { $in: sales.map(row => row._id) } }).select('name category slug images').lean()).map(product => [String(product._id), product]));
        const bestSelling = sales.filter(row => productById.has(String(row._id))).slice(0, 10).map(row => (Object.assign(Object.assign({}, productById.get(String(row._id))), { unitsSold: row.unitsSold, revenue: row.revenue })));
        const soldIds = sales.map(row => row._id);
        const zeroSales = yield product_model_1.default.find(Object.assign(Object.assign({}, activeProducts), (soldIds.length ? { _id: { $nin: soldIds } } : {}))).sort({ updatedAt: -1 }).limit(10).select('name category slug images updatedAt viewCount').lean();
...
router.post('/image-upload-url', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        res.json({ success: true, uploadUrl, imageUrl: `https://${s3_1.S3_BUCKET_NAME}.s3.${region}.amazonaws.com/${key}` });
    }
    catch (error) {
        next(error);
    }
...
router.post('/', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const initialAdjustments = product.sizes.filter(item => item.stock > 0).map(item => (Object.assign({ productId: created._id, productName: created.name, size: item.size, delta: item.stock, beforeStock: 0, afterStock: item.stock, reason: 'Initial inventory', source: 'initial' }, actor(req))));
...
router.patch('/:id', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (removedWithStock)
            return res.status(409).json({ success: false, message: `Adjust ${removedWithStock.size} stock to zero before removing or renaming it.` });
        ...
        const stockChanges = product.sizes.flatMap(item => {
            ...
            return beforeStock === item.stock ? [] : [Object.assign({ productId: updated._id, productName: updated.name, size: item.size, delta: item.stock - beforeStock, beforeStock, afterStock: item.stock, reason: 'Stock changed while editing product', source: 'admin' }, actor(req))];
        });
...
router.get('/:id/stock-adjustments', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [adjustments, total] = yield Promise.all([
            StockAdjustment_1.StockAdjustment.find({ productId: req.params.id }).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(),
...
router.post('/:id/stock-adjustments', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!Number.isInteger(stock) || stock < 0)
            return res.status(400).json({ success: false, message: 'Desired stock must be a whole number that is zero or greater.' });
        ...
        if (!updated)
            return res.status(409).json({ success: false, message: 'Stock changed before it could be saved. Please try again.' });
        yield invalidateCatalog();
        res.json({ success: true, product: updated });
    }
    catch (error) {
...
router.post('/bulk/archive', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const ids = Array.isArray(req.body.ids) ? req.body.ids.filter((id) => typeof id === 'string' && mongoose_1.default.isValidObjectId(id)) : [];
        if (!ids.length)
...
router.delete('/bulk', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (confirmation !== expectedConfirmation)
            return res.status(400).json({ success: false, message: `Type "${expectedConfirmation}" to permanently delete these products.` });
        const retentionDate = new Date(Date.now() - ARCHIVE_RETENTION_DAYS * 24 * 60 * 60 * 1000);
        const eligibleCount = yield product_model_1.default.countDocuments({ _id: { $in: ids }, isDelete: true, archivedAt: { $lte: retentionDate } });
        if (eligibleCount !== ids.length)
            return res.status(409).json({ success: false, message: `Products must stay archived for ${ARCHIVE_RETENTION_DAYS} days before permanent deletion.` });
        const result = yield product_model_1.default.deleteMany({ _id: { $in: ids }, isDelete: true, archivedAt: { $lte: retentionDate } });
        yield invalidateCatalog();
...
router.delete('/:id', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!product)
            return res.status(404).json({ success: false, message: 'Product not found.' });
        if (!product.isDelete || !product.archivedAt || product.archivedAt > new Date(Date.now() - ARCHIVE_RETENTION_DAYS * 24 * 60 * 60 * 1000))
            return res.status(409).json({ success: false, message: `Products must stay archived for ${ARCHIVE_RETENTION_DAYS} days before permanent deletion.` });
        if (String(req.body.confirmationName || '') !== product.name)
            return res.status(400).json({ success: false, message: 'Type the exact product name to permanently delete it.' });
