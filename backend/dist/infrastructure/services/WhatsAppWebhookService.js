                ...
                if (value === null || value === void 0 ? void 0 : value.messages) {
                    for (const message of value.messages) {
                        ...
                        if (type === 'text') {
                            ...
                            const originalText = ((_b = message.text) === null || _b === void 0 ? void 0 : _b.body) || '';
                            ...
                            if (text.startsWith('track ') || text.startsWith('jejak ')) {
                                ...
                                const { chatRepository } = yield Promise.resolve().then(() => __importStar(require('../../infrastructure/repositories/ChatRepository')));
                                let conversation = yield chatRepository.findConversationByWhatsApp(from);
                                if (!conversation) {
                                    // Try to find if we have a user with this phone number
                                    const { default: UserModel } = yield Promise.resolve().then(() => __importStar(require('../../infrastructure/db/models/user.model')));
                                    // the DB phone might or might not have '+' or country code. Let's assume standard format matching
                                    const user = yield UserModel.findOne({ phone: { $regex: new RegExp(from + '$') } });
                                    conversation = yield chatRepository.createConversation({
                                        participants: user ? [user._id] : [],
                                        type: 'admin_customer',
                                        whatsappPhone: from
                                    });
                                }
                                yield chatRepository.createMessage({
                                    conversationId: conversation._id,
                                    senderRole: 'client', // Since it's from WhatsApp
                                    text: originalText,
                                    source: 'whatsapp'
                                });
                                yield chatRepository.updateLastMessage(conversation._id.toString());
                            }
                        }
