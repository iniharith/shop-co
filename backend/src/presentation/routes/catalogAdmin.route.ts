...
const ensureUniqueSlug = async (slug: string, name: string, productId?: string) => {
  ...
  let candidate = base;
  let suffix = 2;
  while (await ProductModel.exists({ slug: candidate, ...(productId ? { _id: { $ne: productId } } : {}) })) {
...
const validateProduct = (product: ReturnType<typeof normalizeProduct>) => {
  ...
  if (product.slug && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(product.slug)) return 'Slug may only contain lowercase letters, numbers, and hyphens.';
  if (product.seoTitle.length > 70) return 'SEO title must be 70 characters or fewer.';
  if (product.seoDescription.length > 160) return 'SEO description must be 160 characters or fewer.';
...
router.get('/analytics', async (_req, res, next) => {
  try {
    ...
    const [mostViewed, sales, lowStock, recentlyUpdated] = await Promise.all([
      ProductModel.find(activeProducts).sort({ viewCount: -1, updatedAt: -1 }).limit(10).select('name category slug images viewCount updatedAt sizes').lean(),
      ...
      ProductModel.find(activeProducts).sort({ updatedAt: -1 }).limit(10).select('name category slug images updatedAt viewCount').lean(),
    ]);
    const productById = new Map((await ProductModel.find({ _id: { $in: sales.map(row => row._id) } }).select('name category slug images').lean()).map(product => [String(product._id), product]));
    const bestSelling = sales.filter(row => productById.has(String(row._id))).slice(0, 10).map(row => ({ ...productById.get(String(row._id)), unitsSold: row.unitsSold, revenue: row.revenue }));
    const soldIds = sales.map(row => row._id);
    const zeroSales = await ProductModel.find({ ...activeProducts, ...(soldIds.length ? { _id: { $nin: soldIds } } : {}) }).sort({ updatedAt: -1 }).limit(10).select('name category slug images updatedAt viewCount').lean();
...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!slug) return res.status(400).json({ success: false, message: 'A product name or slug is required.' });
    const created = await ProductModel.create({ ...product, slug, catalogId: req.body.catalogId || `admin-${randomUUID()}` });
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    const updated = await ProductModel.findByIdAndUpdate(req.params.id, { $set: { ...product, slug } }, { new: true, runValidators: true }).lean();
...
router.post('/:id/stock-adjustments', async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(stock) || stock < 0) return res.status(400).json({ success: false, message: 'Desired stock must be a whole number that is zero or greater.' });
    ...
    if (!updated) return res.status(409).json({ success: false, message: 'Stock changed before it could be saved. Please try again.' });
    await invalidateCatalog();
    res.json({ success: true, product: updated });
  } catch (error) {
    next(error);
...
router.patch('/:id/archive', async (req, res, next) => {
  try {
    ...
    const updated = await ProductModel.findByIdAndUpdate(req.params.id, { $set: { isDelete: archived, archivedAt: archived ? new Date() : null } }, { new: true }).lean();
    if (!updated) return res.status(404).json({ success: false, message: 'Product not found.' });
    await invalidateCatalog();
    res.json({ success: true, product: updated });
  } catch (error) {
...
router.post('/bulk/archive', async (req, res, next) => {
  try {
    ...
    const archived = Boolean(req.body.archived);
    const result = await ProductModel.updateMany({ _id: { $in: ids } }, { $set: { isDelete: archived, archivedAt: archived ? new Date() : null } });
    await invalidateCatalog();
...
router.delete('/bulk', async (req, res, next) => {
  try {
    ...
    if (confirmation !== expectedConfirmation) return res.status(400).json({ success: false, message: `Type "${expectedConfirmation}" to permanently delete these products.` });
    const retentionDate = new Date(Date.now() - ARCHIVE_RETENTION_DAYS * 24 * 60 * 60 * 1000);
    const eligibleCount = await ProductModel.countDocuments({ _id: { $in: ids }, isDelete: true, archivedAt: { $lte: retentionDate } });
    if (eligibleCount !== ids.length) return res.status(409).json({ success: false, message: `Products must stay archived for ${ARCHIVE_RETENTION_DAYS} days before permanent deletion.` });
    const result = await ProductModel.deleteMany({ _id: { $in: ids }, isDelete: true, archivedAt: { $lte: retentionDate } });
    await invalidateCatalog();
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const product = await ProductModel.findById(req.params.id).lean();
    if (!product) return res.status(404).json({ success: false, message: 'Product not found.' });
    if (!product.isDelete || !product.archivedAt || product.archivedAt > new Date(Date.now() - ARCHIVE_RETENTION_DAYS * 24 * 60 * 60 * 1000)) return res.status(409).json({ success: false, message: `Products must stay archived for ${ARCHIVE_RETENTION_DAYS} days before permanent deletion.` });
    if (String(req.body.confirmationName || '') !== product.name) return res.status(400).json({ success: false, message: 'Type the exact product name to permanently delete it.' });
