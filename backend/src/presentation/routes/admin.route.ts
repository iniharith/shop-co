...
router.get("/migrate-statuses", asyncHandler(async (req, res) => {
  try {
    ...
    const o3 = await Order.collection.updateMany({ orderStatus: "HOLD_PRINTING" }, { $set: { orderStatus: "PRINT_AWB" } });
    ...
    const t6 = await Task.collection.updateMany({ status: "HOLD_PRINTING" }, { $set: { status: "PRINT_AWB" } });
