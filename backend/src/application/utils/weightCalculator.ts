...
const sizeMap: Record<string, SizeInfo> = {
  'A3': { area: 0.12474 },
  'A4': { area: 0.06237 },
  'A5': { area: 0.03108 },
  'A6': { area: 0.01554 },
...
export const calculateOrderTotalWeight = (products: any[]): number => {
  let totalWeight = 0;
  for (const item of products) {
    ...
    const w = calculateItemWeight(item.quantity || 1, item.size || 'A4', item.material || '');
    totalWeight += w;
  }
